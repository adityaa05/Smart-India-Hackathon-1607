...
class TrafficSignalGUI:
    ...
    def simulate_traffic_spike(self):
        for lane in self.lanes:
            self.lanes[lane] += int(psutil.cpu_percent() * 2)  # Use CPU usage to generate random spike
        self.display_message("Traffic spike simulated")
        self.submit_counts()

    def clear_historical_data(self):
        self.controller.historical_data.clear()
        open('historical_data.csv', 'w').close()  # Clear the CSV file
