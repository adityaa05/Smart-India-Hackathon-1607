...
class TrafficSignalController:
    ...
    def log_signal_change(self, lane, color, duration):
        """Log signal changes to a CSV file."""
        with open('traffic_signal_log.csv', mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([datetime.now().strftime('%Y-%m-%d %H:%M:%S'), lane, color, duration])

    def update_signals(self, remaining_time, signal_callback, prompt_vehicle_counts):
        ...
        for remaining in range(int(allocated_time), 0, -1):
            if remaining <= 5:  # Last 5 seconds will turn yellow
                ...
                self.log_signal_change(green_lane, "yellow", remaining)
            else:
                signal_callback(green_lane, "green", remaining)
                self.log_signal_change(green_lane, "green", remaining)

            time.sleep(1)

        # After the loop, turn the signal red
        signal_callback(green_lane, "red")
        self.log_signal_change(green_lane, "red", 0)  # Log red signal change with 0 duration
    ...
    def run_cycle(self, signal_callback, prompt_vehicle_counts):
        remaining_time = self.total_cycle_time

        while remaining_time > 0 and len(self.omitted_lanes) < len(self.lanes):
            allocated_time = self.update_signals(remaining_time, signal_callback, prompt_vehicle_counts)
            remaining_time -= allocated_time

    def start(self, signal_callback, prompt_vehicle_counts):
        while True:
            self.run_cycle(signal_callback, prompt_vehicle_counts)
...
class TrafficSignalGUI:
    ...
    def start_traffic_signal(self):
        ...
        def run_signal():
            self.controller.start(self.update_signal, self.prompt_vehicle_counts)
