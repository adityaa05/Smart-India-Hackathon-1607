...
class TrafficSignalController:
    def __init__(self, lanes, update_signal_callback, update_progress_callback, total_cycle_time=120):
        ...
        self.blinking_lock = threading.Lock()  # Lock for blinking
    ...
    def blink_yellow(self, lane):
        ...
        with self.blinking_lock:  # Ensure only one blinking occurs at a time
...
class TrafficSignalGUI:
    def __init__(self, root, controller):
        ...
        self.cpu_usage_label = None  # CPU usage label
        self.active_threads_label = None  # For thread management
        self.create_widgets()
        ...
        self.start_cpu_monitoring()  # Start CPU monitoring
    ...
    def load_historical_data(self):
        try:
            with open('historical_data.csv', 'r') as csvfile:
                ...
                for row in reader:
                    self.historical_display.insert(tk.END, f"Lane: {row[0]}, Time Allocated: {row[1]}s, Timestamp: {row[2]}\n")
                self.historical_display.config(state=tk.DISABLED)  # Disable after loading
