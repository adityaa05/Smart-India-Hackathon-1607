...
class TrafficSignalController:
    def __init__(self, lanes, update_signal_callback, update_progress_callback, total_cycle_time=120):
        self.lanes = lanes
        self.update_signal_callback = update_signal_callback  # Store the signal update callback
        self.update_progress_callback = update_progress_callback  # Store the progress update callback
    ...
    def update_signals(self, remaining_time, prompt_vehicle_counts):
        ...
        for remaining in range(int(allocated_time), 0, -1):
            ...
            self.update_progress_callback(green_lane, remaining, allocated_time)  # Use the callback for progress
...
def run_gui():
    ...
    root.geometry("725x400")  # Set a fixed size for the window

    lanes = {"North": 0, "East": 0, "South": 0, "West": 0}
    controller = TrafficSignalController(lanes, None, None)  # Initialize without callbacks first

    gui = TrafficSignalGUI(root, controller)

    # Pass the update_signal and update_progress methods to the controller
