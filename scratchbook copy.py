...
class TrafficSignalController:
    ...
    def update_signals(self, remaining_time, prompt_vehicle_counts):
        ...
        for remaining in range(int(allocated_time), 0, -1):
            ...
            if remaining <= 5 and remaining > 0:  # Added check for remaining > 0
                self.update_signal_callback(green_lane, "yellow", remaining)  # Update signal to yellow
                playsound.playsound(playsound.default_sound)  # Play sound
            time.sleep(1)
...
class TrafficSignalGUI:
    ...
    def update_entry_fields(self):
        for lane in self.lanes:
            # Update the entry fields to reflect the new counts after the spike
    ...
    def update_signal(self, lane, color, remaining=None):
        if self.root.winfo_exists():
            ...
            if color == "yellow" and remaining is not None and remaining <= 5:
                playsound.playsound(playsound.default_sound)  # Play system sound for yellow blinking
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    root.title("Traffic Signal Controller - Developer Edition")
