...
class TrafficSignalController:
    ...
    def update_signals(self, remaining_time, signal_callback, prompt_vehicle_counts):
        ...
        signal_callback(green_lane, "green")
        
        # Countdown logic
        for remaining in range(int(allocated_time), 0, -1):
            signal_callback(green_lane, "green", remaining)  # Pass remaining seconds
            time.sleep(1)

        signal_callback(green_lane, "yellow")
        prompt_vehicle_counts()  # Enable input 5 seconds before the green light ends
        time.sleep(5)  # Wait 5 seconds for input
...
class TrafficSignalGUI:
    ...
    def update_signal(self, lane, color, remaining=None):
        ...
        if remaining is not None:
            self.lane_frames[lane].config(text=f"{color.capitalize()} ({remaining}s)")
        else:
