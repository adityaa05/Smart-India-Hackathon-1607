...
class TrafficSignalGUI:
    def __init__(self, root, controller):
        ...
        self.historical_display.grid(row=5, column=0, columnspan=4)
    ...
    def display_message(self, message):
        if self.root.winfo_exists():
            ...
            self.historical_display.see(tk.END)
            self.historical_display.config(state=tk.DISABLED)
    ...
    def load_historical_data(self):
        try:
            with open('historical_data.csv', 'r') as csvfile:
                ...
                for row in reader:
                    self.historical_display.config(state=tk.NORMAL)
                    self.historical_display.insert(tk.END, f"{row}\n")
                self.historical_display.config(state=tk.DISABLED)
        except FileNotFoundError:
            self.display_message("No historical data found.")
